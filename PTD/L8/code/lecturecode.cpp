...
#include <QDebug>
  ...
  MyThread::MyThread(QObject * parent, int _v) : QThread(parent)
  {
    data = _v;
  }
...
#include "Logger.h"
  ...
  int main()
  {
    ...
    auto funce = std::bind(static_cast<void (Foo::*)(const std::string &, const std::string &)>(&Foo::foo), b, sa, sb);
    threads.emplace_back(funce);
    auto funcf = std::bind(static_cast<void (Foo::*)(const std::string &, const std::string &)>(&Foo::foo), pFoo.get(), sa, sb);
...
#include <numeric>
  ...
  int sumVectLambda(const std::vector<int> &v)
  {
    std::cout << "sumLambda\n";
    int sum = 0;
    for_each(std::begin(v), std::end(v), [&sum](int x)
             { sum += x; });
    return sum;
  }
  int main()
