...
- It is fine to use them but you should use caution as the code is sometimes not what you need
	- I tend to use it as a better search system (I will demonstrate this from time to time)
...
- g++ is similar and is based on the GNU compiler collection
...
- It also uses a meta language to generate a makefile or other build system
- Many open source projects use this (especially some of big animation packages such as OSL, OpenEXR etc etc)
- Uses a file called CMakeLists.txt to generate the build file
- we will use this to generate a Ninja build file
